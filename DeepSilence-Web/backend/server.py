...
from utils.sqlite_operator import delete_user_submission
...
@app.route('/clear', mathods = ['GET'])
def clear_records():
    account = request.args.get('account')
    conn = create_connection(database)
    delete_user_submission(conn, account)
    current_path = os.path.abspath(os.getcwd())
    for item in os.listdir(f"./user_upload/{account}/ori"):
        os.remove(os.path.join(current_path, f"./user_upload/{account}/ori/{item}"))
    for item in os.listdir(f"./user_upload/{account}/adv"):
        os.remove(os.path.join(current_path, f"./user_upload/{account}/adv/{item}"))
