...
def get_recursive_files(folderPath, ext):
    ...
    for file in results:
        if os.path.isdir(os.path.join(folderPath, file)):
            outFiles += get_recursive_files(
                os.path.join(folderPath, file), ext)
    ...
    for i, audio in enumerate(input_audios):
        ...
        if is_monphonic:
            ...
            D = librosa.amplitude_to_db(
                np.abs(librosa.stft(audio[0])), ref=np.max)
...
def visualize_loss(loss_1, loss_2, first_legend, second_legend, y_label):
    ...
    plt.title("{} and {} Loss During Training".format(
        first_legend, second_legend))
...
def sample_audio(audio_data, start_idx=None, end_idx=None):
    ...
    if audio_len == window_length:
        ...
        if start_idx is None or end_idx is None:
            diff = (audio_len - window_length)
            # diff = (audio_len - window_length) // 2
            bias_range = min(diff, reverb_bias * sampling_rate)
            start_idx = np.random.randint(0, bias_range)
...
def save_samples(save_dir, epoch_samples, epoch):
    """
    Save output samples.
    """
    sample_dir = make_path(os.path.join(save_dir, str(epoch)))
    ...
    for idx, sample in enumerate(epoch_samples):
        ...
        if idx >= num_save_samples:  # * less sample saved
            break
...
class RirDataLoader:
    ...
    def __getitem__(self, idx):
        wav = load_wav(self.signal_paths[idx])
        return self.numpy_to_tensor(wav)
